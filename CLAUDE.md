...
└── src/
    └── GLoom.Plugin/
        ...
        ├── Mcp/                        (the agent-facing endpoint — see the experiment/mcp section)     [v0.3.0-mcp.3]
        ...
        │   └── Tools/Live/             (pure: LiveModels + ILiveHost = the runtime-sidecar contract; LiveTools
        │                                (documents, read_document, read_outputs, solve, rhino_context), CatalogueTools
        │                                (catalogue, canvas_image); tests drive them through FakeLiveHost)
        ├── Serialization/
...
### experiment/mcp — G-Loom as an MCP server (v0.3.0-mcp.3, rungs 1–3 of 8)

**Status: rungs 1 and 1b passed on Windows 2026-08-26, inside Rhino.Inside.Revit, which clears the load gate and rung 1 in the hardest host at once. Rung 2 (the memory lane) passed its Windows smoke test the same day: `/mcp` lists 9 tools, the `@gloom:` picker serves the resources, and `gloom_diff` matched the panel drawer on a real definition. Rung 3 (live read via `UiThread`) is built and unit-tested at the tool layer (2026-08-27); its Windows smoke test — an error component reported, a modal dialog timing out cleanly, a screenshot with ink — is the next job.** Branches from `experiment/canvas-components`. The full plan, catalogue and execution protocol live in the session plan that created the branch; this section is the in-repo summary.
