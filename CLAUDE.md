...
└── src/
    └── GLoom.Plugin/
        ...
        ├── Mcp/                        (the agent-facing endpoint — see the experiment/mcp section)     [v0.3.0-mcp.4]
        ...
        │   │                            Live/ = LiveHost + LiveCatalogue + LiveCanvasImage + GooText, the ILiveHost
        │   │                            implementation: every method one UiThread.Run over Grasshopper/Rhino)
        ...
        │   │                            ObjectFilter (the kind/query filter both tool families share))
        │   └── Tools/Live/             (pure: LiveModels + ILiveHost = the runtime-sidecar + value-edit contract; LiveTools
        │                                (documents, read_document, read_outputs, solve, rhino_context), CatalogueTools
        │                                (catalogue, canvas_image), ValueTools (set_value, restore_objects);
        │                                tests drive them all through FakeLiveHost)
        ├── Serialization/
...
### experiment/mcp — G-Loom as an MCP server (v0.3.0-mcp.4, rungs 1–5 of 8)

**Status: rungs 1, 1b and 2 passed on Windows 2026-08-26 — rung 1's whole exercise ran inside Rhino.Inside.Revit, clearing the load gate and rung 1 in the hardest host at once. Rungs 3, 4 and 5 were built on 2026-09-03: 24 tools, 143 test cases, zero warnings. Their Windows smoke test is the next job. Rung 3 had in fact been written on 2026-08-27 and left uncommitted in a second checkout for a week; recovering it was this session's first act, and is why `docs/MEMORY.md` now insists on pushing after every commit rather than only committing.** Branches from `experiment/canvas-components`. The full plan, catalogue and execution protocol live in the session plan that created the branch; this section is the in-repo summary.
...
**What exists (rung 3, live read):** the first host-bound rung, split so the host stays thin and the tools stay testable.

- **The contract.** `Mcp/Tools/Live/LiveModels.cs` + `ILiveHost.cs` are the runtime sidecar as plain records: per object its message level and messages, locked/hidden, solve phase, processor time, input/output data counts with a preview; per document its solve state; plus outputs as data trees, solve reports, catalogue entries, a PNG, the Rhino context. `Mcp/Host/Live/LiveHost.cs` implements it — **every method is exactly one `UiThread.Run`** that does all Grasshopper/Rhino work on the UI thread and returns only records; nothing Grasshopper-typed crosses to the request thread. `LiveTools`/`CatalogueTools` shape, page, validate and map `TimeoutException` to a tool error the agent can act on ("a modal dialog or a long solve is holding the UI thread"). Tests drive the tools through `FakeLiveHost`.
- **Tools.** `gloom_documents` (open definitions, active one, saved/enabled/solved, error and warning counts, project root); `gloom_read_document` (the LIVE canvas, unsaved edits included: recipe fields joined with the runtime sidecar by InstanceGuid, `problems` listed up front regardless of paging, `onlyProblems`/`query`/`kind` filters, output previews); `gloom_read_outputs` (one object's outputs as data trees, branch by branch, geometry as type + bounding box; an object is an instanceGuid — including an input/output port's — or an exact name/nickname, ambiguity refused with candidates); `gloom_solve` (**Write**; `NewSolution(expireAll, GH_SolutionMode.CommandLine)` with the caller's timeout; refuses a locked solver, a disabled document — Grasshopper disables background tabs — and a running solution); `gloom_catalogue` (categories → `FindObjects` fuzzy search ranked by Grasshopper's own weights, or a whole category paged, or `describe` a guid via `CreateInstance` for its inputs/outputs); `gloom_canvas_image` (`GetCanvasScreenBuffer` for the visible view, `GenerateHiResImageTile` through a `GH_Viewport` aimed by `Target` for `all`/`objects`; text summary first, PNG second; ≤ 3 MB; active tab only); `gloom_rhino_context` (Rhino/GH/RiR/G-Loom versions, host process, active model units/tolerance/counts).
- **SDK notes for future rungs:** `GH_ComponentServer.FindObjects` takes its result arrays by `ref`, not `out`; `GH_Viewport.Target` is the pixel where the canvas origin lands (`VisibleRegion = -Target / Zoom`) and `MidPoint`'s setter uses the live zoom, so frame by setting `Target` after `Zoom`; `GetCanvasScreenBuffer` returns a fresh bitmap (dispose it); `%(contents)` of a lightweight tag is the commit message.

**What exists (the write lane, landed 2026-09-02 ahead of its rung):** four `ToolAccess.Write` tools in `Mcp/Tools/Memory/WriteTools.cs`, host-free, reusing the panel's git primitives through delegates injected by `McpService` — `gloom_commit` (saves the canvas if dirty, serializes, stages the pair, commits with the auto `Gloom-Version` trailer plus `Gloom-Agent`/`Gloom-Intent`, unstages on any failure), `gloom_revert` (restores a version's files and reloads the canvas from disk), `gloom_switch_branch` (adopts a system option and reloads every open definition in the repo), `gloom_tag` (pins a milestone with the running toolchain captured into the annotated tag message). **Commits stay the human's**: the author comes from `git config user.name/user.email` via `GLoomRepository.ConfiguredIdentity`, which refuses with a readable hint when unset, and the agent is named only in trailers. Every success path ends in a forced `DocumentTracker.Refresh()` + `NotifyExternalChange()`, because a pure git commit fires no document event and the panel would otherwise sit stale.

**What exists (rung 4, the edit envelope):** the leg that makes this branch the review layer rather than another way to script Grasshopper. The edits worth reviewing need not come through G-Loom at all — McNeel's server can drive the same canvas — so the guarantee is not "every mutation is recorded" but **"there is always a version to go back to"**, which a checkpoint gives whoever did the editing.

- **`Mcp/State/EditEnvelope.cs`** — the envelope (repo, definition, checkpoint sha, agent, session, intent, opened-at) and `EnvelopeStore`, a whole-file store under `<appdata>/G-Loom/mcp/envelope-<pid>.json`. On disk because the endpoint is stateless and each request arrives on its own thread; a file left behind after a crash is how the next session learns an envelope was abandoned rather than closed.
- **`gloom_begin_edit`** commits the human's unsaved work *first* so the checkpoint is a real version rather than a sha with loose changes above it, then aims `CanvasDiffOverlay.SetComparisonReference` at it. That second part is the point: from there the person in front of Rhino watches the agent's changes light up on their canvas as they land, and rejects any one of them by right-clicking — the review screen already existed, this only aims it. **`gloom_end_edit`** commits in the human's name with `Gloom-Agent` / `Gloom-Agent-Session` / `Gloom-Intent` / `Gloom-Checkpoint-Base`, or `discard`s back to the checkpoint (through `reloadFromDisk`, so the canvas can never keep edits git no longer has). A second `begin_edit` is refused naming the holder and its intent.
- **`gloom_set_value`** (`Mcp/Tools/Live/ValueTools.cs` + `Host/Live/LiveValues.cs`) writes sliders, panels, toggles, value lists and colour swatches as one undoable step, batched so N edits cost one solve, reporting before and after per object. Only those five: gradients and MD sliders are *read* by reflection over third-party types, so writing them back would mean invoking reflected methods — the thing that broke Grasshopper's drawing pipeline once — and internalised data is stored only as a digest. All three refuse out loud instead of silently no-opping.
- **`gloom_restore_objects`** puts named objects back as a version had them, defaulting to the open envelope's checkpoint: value and position reset where they still exist, recreated with their original identity and rewired where they were deleted. Per-component rejection, driven by an agent instead of a right-click. It replays recorded history rather than authoring freely — that is the line that keeps graph authoring with McNeel's server.
- **Both refuse unless an envelope is open.** A value change is trivial to make and easy to lose track of; the checkpoint is what makes it safe.
- **`Ui/DocumentRestore.cs`** — the restore primitives lifted out of `CanvasDiffOverlay` unchanged, so the right-click and the agent path are the same code. `ApplyPersistent` gained value lists (single and multi-selection) and now returns a reason instead of silently doing nothing: restoring a changed value list used to *appear* to work.
- **The panel gained an `Agent editing:` row** naming who is mid-edit, on which definition, since when, with **Clear** for an envelope an agent left behind. Undoing the work stays the history list's job, where the checkpoint sits as an ordinary version with a Restore button.

**Two defects in shipped code that this rung's verification turned up, fixed first:** `UiThread.Run` waited with `Task.Wait`, which wraps a faulted task in an `AggregateException` — so the dispatcher's `ToolArgumentException` case never matched and every refusal raised on the UI thread reached the agent as *"failed: One or more errors occurred."*; it now waits on the completion handle so `GetResult` rethrows the original type. And the panel staged its `.gh`/`.gloom.json` pair across a modal dialog while git's index is repo-wide, so a `gloom_commit` in that window committed the human's files under the agent's subject and the panel then reported "nothing to commit" for a commit that had happened — `Vcs/IndexGate.cs` is an interlocked, any-thread-readable flag held by the panel across stage→commit and by the three MCP tools that write the index. Any-thread on purpose: checking it through `UiThread.Run` would wait out the full timeout during exactly the long solve it exists to stay clear of.

**Also folded in:** `Vcs/Identity.cs` is now the single commit identity for both the panel (which hardcoded a name and email at all three write sites) and the MCP path; the history drawer parses trailers and names the agent and intent on their own line, where before it showed them as raw prose and the human reviewing an agent could not see the attribution the agent itself could read; and `gloom_revert` is registered `ToolAccess.Destructive`, so `destructiveHint` is no longer false for every tool G-Loom ships.

**What exists (rung 5, the agent plugin):** `agent/gloom/` — `.claude-plugin/plugin.json`, `.mcp.json` (HTTP transport, `Authorization: Bearer ${GLOOM_MCP_TOKEN}`, 600 s timeout to match the client default), and three skills. `gloom-project-memory` teaches the workflow rather than the tool list — status first, read the record before proposing, envelope-and-intent before touching the canvas, and the project's own vocabulary so an agent says *system option* and *milestone*. `grasshopper-docs` sends an agent to `gloom_catalogue` before the internet, because it answers from the Rhino actually running with the `componentGuid` placement needs; single pages on demand, never a crawl. `gloom-plugin-dev` is the build/deploy/test loop and the invariants that exist because something broke. McNeel's `Rhino-MCP-Platform` is **documented** as the second `.mcp.json` entry rather than configured, since it ships its own installer and defaults and guessing them would only go stale.
...
| 3 | live read via `UiThread`: documents, `read_document` (+ runtime sidecar), `read_outputs`, `solve`, catalogue (`ObjectProxies` + `CreateInstance`), `canvas_image` (`GetCanvasScreenBuffer`), `rhino_context` | **Built 2026-08-27, committed 2026-09-03** — 123 test cases; Windows smoke test pending: error component reported; a call with a modal open times out cleanly; screenshot has ink |
| `experiment/mcp` | **The MCP endpoint** — G-Loom as the memory / provenance / review layer for coding agents (`Mcp/`, `Ui/DocumentRestore`, `Vcs/{CommitTrailers,Identity,IndexGate,GLoomLog}`, `agent/gloom/`, `tests/GLoom.Core.Tests`). Branches *from* canvas-components. | `v0.3.0-mcp.4` (pre-release) | **Rungs 1, 1b, 2 passed on Windows 2026-08-26; rungs 3, 4, 5 built 2026-09-03, smoke test pending.** See the section above. |
| 5 | `agent/gloom/` Claude Code plugin: `.mcp.json` + skills `gloom-project-memory`, `grasshopper-docs` (fetch single pages on demand — the site's ToS forbids crawling; `gloom_catalogue` first for anything installed), `gloom-plugin-dev` | **Built 2026-09-03** — McNeel's server documented as the second entry rather than guessed; pending: a fresh session installs it and reviews an agent edit end to end |
...
| `experiment/mcp` | **The MCP endpoint** — G-Loom as the memory / provenance / review layer for coding agents (`Mcp/`, `Vcs/CommitTrailers`, `Vcs/GLoomLog`, `tests/GLoom.Core.Tests`). Branches *from* canvas-components. |  `v0.3.0-mcp.4` (pre-release) | **Rungs 1, 1b and 2 passed on Windows (2026-08-26); rung 3 built 2026-08-27, smoke test pending.** See the section above. |
