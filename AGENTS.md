...
| `experiment/mcp` | **MCP endpoint** — G-Loom as the memory/provenance/review layer for coding agents. `Mcp/` (Protocol/State/Host/Tools-Memory), `Vcs/CommitTrailers`, `Vcs/GLoomLog`, `tests/GLoom.Core.Tests`. Branches from canvas-components. | `v0.3.0-mcp.2` (pre-release) | Rungs 1, 1b, 2 passed on Windows 2026-08-26; rung 3 (live read via `UiThread`) next. |
