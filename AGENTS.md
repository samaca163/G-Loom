...
| `experiment/mcp` | **MCP endpoint** — G-Loom as the memory/provenance/review layer for coding agents. `Mcp/` (Protocol/State/Host + Host/Live, Tools/Memory + Tools/Live), `Vcs/CommitTrailers`, `Vcs/GLoomLog`, `tests/GLoom.Core.Tests`. Branches from canvas-components. | `v0.3.0-mcp.3` (pre-release) | Rungs 1, 1b, 2 passed on Windows 2026-08-26. Rung 3 (live read) + the write lane are in; 20 tools, 123 test cases; Windows smoke test pending. |
